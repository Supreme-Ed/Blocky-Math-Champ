 ...
 * @param {BABYLON.Mesh} mesh - The original cube mesh
 * @param {BABYLON.Vector3} avatarPosition - The target position for the animation
 * @param {object} [options] - Optional animation options (duration, etc)
 * @returns {Promise<void>}
 */
export async function playCubeFlyToAvatarEffect(mesh, avatarPosition, options = {}) {
  ...
  await scene.beginAnimation(clone, 0, duration, false, 1, () => {
    clone.dispose();
  });
}