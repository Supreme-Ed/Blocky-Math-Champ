// src/components/CubePlatform.ts
// Modular Babylon.js cube platform with math answer rendering and procedural texture fallback
import { loadBlockTexture, getBlockTypeById } from '../game/blockTypes';
import { normalMapFromHeightmap } from './normalMapFromHeightmap';
import {
  MeshBuilder,
  Vector3,
  StandardMaterial,
 ...
 *
 * @param params - Parameters for creating the cube platform
 * @param params.scene - Babylon.js scene
 * @param params.blockTypeId - Block type ID (e.g., 'grass', 'stone')
 * @param params.answer - Math answer to display
 * @param params.position - { x, y, z } position for the cube
 * @param params.size - Size of the cube (default: 2)
 * @returns The created cube mesh
 */
export async function createCubePlatform({
  scene,
  blockTypeId,
  answer,
  position,
  size = 2
}: CubePlatformParams): Promise<Mesh> {
  ...
  // Create a temporary canvas to generate the normal map
  const tempCanvas = document.createElement('canvas');
  tempCanvas.width = 256;
  tempCanvas.height = 256;
  const tempCtx = tempCanvas.getContext('2d');

  // Create a temporary image from the heightmap texture
  const tempImage = new Image();
  tempImage.src = heightmapTex.getContext().canvas.toDataURL('image/png');
  ...
  const normalMapCanvas = normalMapFromHeightmap(tempCanvas, 2.0);
  ...
  const blockDiffuseTexture = await loadBlockTexture({
    ...
    procedural: 'GrassProceduralTexture', // Default procedural texture
    scene,
  });

  // Disable smoothing (nearest neighbor filtering) for pixelated look
  if (blockDiffuseTexture && typeof blockDiffuseTexture.updateSamplingMode === 'function') {
  ...
  if (box.subMeshes) {
    box.subMeshes.forEach(sm => sm.dispose());
    box.subMeshes = [];
  }
  ...
  // Note: We don't need to explicitly add it to the shadow generator here
  // because we're using the onNewMeshAddedObservable in BabylonSceneContent.tsx
